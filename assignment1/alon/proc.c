...
scheduler(void)
{
  ...
  for(;;){
	...
	default:			// the deafult RR case stayed as it was
	  ...
	  break;
	case _FRR:
FRR:	  t->quanta = QUANTA;			// give the process a quanta for his executing
	  if(!frr_min)				// we init frr_min=0 at each iteration
	  {
	    frr_min = t->qvalue;
	    medium = t;
	  }
	  else if(t->qvalue < frr_min)		// find min queue value
	  {
	    frr_min = t->qvalue;
	    medium = t;				// medium we'll be the process that we execute if no high priority process is found
	  }
	  break;
	case _GRT:
GRT:	  if(t->ctime!=currentime)
	  ...
	  else if(t->qvalue < grt_min)				// find min queue value
	  {
	    grt_min = t->qvalue;
	    high = t;
	  }
	  break;
	case _3Q:
	  if(t->priority == HIGH || t->priority == 0)		// mark that we found a high priority process. priority=0 is for the init process and
	  {							// jump to the GRT case
	    highflag = 1;
	    goto GRT;
	  }
	  else if(t->priority == MEDIUM)			// mark that we found a medium priority process and jump to the FRR case
	  {
	    mediumflag = 1;
	    goto FRR;
	  }
	  else if(!lowflag && t->priority == LOW)		// mark that we found a low priority process
	  {
	    head=t;						// head will now point to the low priority process that was found
	    lowflag = 1;
    ...
    if(SCHEDFLAG != _RR)					// if we're not in the deafult case
    {
      if(SCHEDFLAG == _FRR)					// choose the process to be executed according to the highest priority that was found
	p = medium;
...
yield(void)
{
  ...
  {
    case _FRR:     
      acquire(&tickslock);			// after executing a process we'll update his queue value to be ticks so it'll be at the end 
      proc->qvalue = ticks;			// end of the queue
      release(&tickslock);
      break;
    case _GRT:
      break;
    case _3Q:
      if(proc->priority == MEDIUM)		// after executing a process  in the FRR policy we'll update his queue value to be ticks so it'll
